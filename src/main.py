...
class Game:
    ...
    def update(self):
            ...
            if self.ai_mode_active:
                ...
                if self.solution_paths:
                    ...
                    if self.player.move(dx, dy, self.maze, self.maze.cell_size):
                        ...
                        if self.player_algo in ["BFS", "IDS", "DFS", "PO_search", "AND_OR","Non_infor", "Forward Checking"]:
                            self.is_player_turn = True
                        else:
                            self.is_player_turn = False
                    else:
        ...
        # SỬA: Sử dụng tên chính xác
        if self.player_algo not in ["BFS", "IDS", "DFS", "PO_search", "AND_OR","Non_infor", "Forward Checking"]:
