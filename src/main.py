...
class Game:
    def __init__(self):
        ...
        self.arrow_images = {
            "UP": pygame.transform.scale(pygame.image.load(os.path.join(IMAGES_PATH, "up_arrow.png")).convert_alpha(),
                                         size=(self.maze.cell_size, self.maze.cell_size)),
        ...
        self.available_maps = sorted(
            [f for f in os.listdir(MAPS_PATH) if not f.endswith('_agent.txt')],
    ...
    def set_buttons(self):
        ...
        def change_map():
            try:
                # Mở popup chọn map
                map_popup = MapPopup(self)
                selected_map = map_popup.show()

                # Nếu chọn map khác với map hiện tại thì load map mới
                if selected_map and selected_map != self.maze.map_name:
                    self.load_new_map(selected_map)
                    btn_change_map.text = selected_map
        ...
        btn_mummy_toggle = Button(btn_x, 150, btn_w, btn_h, f"Mummy: {'ON' if self.mummy_enabled else 'OFF'}",
                                  toggle_mummy)
    ...
    def update(self):
        ...
        if self.mummy_enabled and not self.is_player_turn and not self.player.is_moving:
            ...
            if current_mummy.path:
                ...
                print(
                    f"Mummy {self.current_mummy_index + 1} at ({current_mummy.grid_x}, {current_mummy.grid_y}) moves {action}")
