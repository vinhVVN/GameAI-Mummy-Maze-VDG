...
class Button:
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEMOTION: 
            self.is_hovered = self.rect.collidepoint(event.pos) # xác định xem điểm pos (chuột) có nằm trong rect hay ko
...
class Panel:
    ...
    def draw(self, surface):
        ...
        tilte_rect = title_surf.get_rect(center=(self.rect.centerx, self.rect.top + 40))
        surface.blit(title_surf, tilte_rect)
